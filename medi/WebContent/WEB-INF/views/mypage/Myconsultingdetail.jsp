		...
		<section id="sub_section">
			...
			<div class="content"> 
				...
				<!-- 이부분에 컨텐츠 시작 -->
					...
					<input type="hidden" name="mem_seq" id="seq" value="${bbs.seq}"/><!-- 질문의seq 29-->
					<c:if test="${not empty login }">
					<input type="hidden" name="seq" id="seq1" value="${login.seq}"/><!-- 로그인한회원seq 9 -->
					</c:if>
					...
					<table>
						<c:forEach items="${answerlist}" var="ansbbs" varStatus="vs">
							...
							<c:if test="${bbs.selectyn eq 0} ">	<!-- 채택아직안된글 -->
								...
								<a href="#none" onclick="mysaveselected()" class="btn-type02 btn-search refresh_btn1" style="cursor:pointer; width:140px; margin-right: 50px;">
							...
							<a href="#none" onclick="myanswerupdate(${ansbbs.ans_seq })" class="btn-type02 btn-search refresh_btn1" style="cursor: pointer; width: 140px; margin: 20px;"><em style="padding:0px;">답글수정하기</em></a>
							<a href="#none" onclick="myanswerdelete(${ansbbs.ans_seq })" class="btn-type02 btn-search refresh_btn1" style="cursor: pointer; width: 140px;  margin: 20px;"><em style="padding:0px;">답글삭제하기</em></a>
							</c:if>
...
$("#_btnUpdate").click(function() {	
	...
	$("#_frmForm").attr({ "target":"_self", "action":"Myconsultingupdate.do" }).submit();
});
$("#_btnDelete").click(function() {	
	var _form = new FormData(document.getElementById('_frmForm'));
    $.ajax({
       url: 'myconsultingdelete.do',
       data: _form,
       dataType: 'text',
       processData: false,
       contentType: false,
...
$("#_btnReply").click(function() {	
	/* alert('답글달기');	 */
	$("#_frmForm").attr({ "target":"_self", "action":"myanswerwrite.do" }).submit();
});
